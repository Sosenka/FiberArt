        ...
        for (i = 0; i < data.villages.length; i++){
            if (data.villages[i].character_name == null){
                    ...
                    lista[i] = data.villages[i].id;
                        ...
                        injector.get('socketService').emit({type:'Scouting/recruit' }, {village_id: lista[i], slot:2} );
                        injector.get('socketService').emit({type:'Scouting/recruit' }, {village_id: lista[i], slot:3} );
                        injector.get('socketService').emit({type:'Scouting/recruit' }, {village_id: lista[i], slot:4} );
                        injector.get('socketService').emit({type:'Scouting/recruit' }, {village_id: lista[i], slot:5} ); 
            }
        }
    });